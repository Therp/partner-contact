...
class ResPartner(Model):
    ...
    def _create_relation_type_tab(self, rel_type, inverse, field_names):
        ...
        onchange_type_values = self.env['res.partner.relation']\
            .on_change_type_selection_id(None,
                                         rel_type.id * 10 +
                                         (1 if inverse else 0))
    ...
    def _add_relation_type_tab(
            self, rel_type, inverse, field_names, relation_tab):
        '''add the xml node to the view'''
        tab = self._create_relation_type_tab(rel_type, inverse, field_names)
            ...
            for rel_type in own_tab_types:
                if rel_type.own_tab_left:
                    self._add_relation_type_tab(
                        rel_type, False, field_names, relation_tab)
                if rel_type.own_tab_right:
                    self._add_relation_type_tab(
                        rel_type, True, field_names, relation_tab)

            result['arch'], fields = self\
                ._BaseModel__view_look_dom_arch(view, result['view_id'])
