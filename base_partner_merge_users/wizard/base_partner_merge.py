...
class MergePartnerAutomatic(models.TransientModel):
    ...
    def _merge(self, partner_ids, dst_partner=None, extra_checks=True):
        ...
        if not partner_ids_rs or len(partner_ids_rs) < 1:
            return
        if dst_partner.exists():
            final_partner = dst_partner
        else:
            final_partner = self._get_ordered_partner(partner_ids_rs.ids)[-1]
        # Safety: final partner might have been deleted if something extreme happened,
        # but in normal core flow it's the surviving one.
        if not final_partner or not final_partner.exists():
            return
